...
# This code will give me nightmares.
...
def apply_rules(
    ...
    alive_rule is a list of numbers. Each number represents how many neighbors a living
    cell needs in order to stay alive for the next generation.

    dead_rule is a list of numbers. Each number represent how many neighbors a dead cell
    needs in order to come to life in the next generation.
    """

    if cell == alive_char:
        if neighbors_num in alive_rule:
            updated_grid[y][x] = alive_char
        else:
            updated_grid[y][x] = dead_char
    else:
        if neighbors_num in dead_rule:
            updated_grid[y][x] = alive_char
...
def main():
    ...
    if correct_input(lambda: input("Enter numbers for column and row amounts? (y for yes, anything else for no))): ")) == 'y':
    ...
    # DEAD_RULE = [3]
    #
    # ALIVE_RULE determines how many neighbors are needed for a living cell to stay alive.
    # DEAD_RULE determines how many neighbors are needed for a dead cell to come to life.
    ALIVE_RULE = [2, 3]
    DEAD_RULE = [3]

    if correct_input(lambda: input("Empty or randomized grid? (r for randomized, anything else for empty): ")) == "r":
...
# The above function as well as the following code exists so that you can
# import the above functions into a another program (if you want to) more
# easily.
